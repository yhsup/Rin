...
export function MarkdownEditor({ content, setContent, placeholder = "> 开始书写...", height = "600px" }: any) {
  ...
  const checkStyleStatus = useCallback((editorInst: editor.IStandaloneCodeEditor) => {
    ...
    const check = (tag: string, endTag?: string) => {
      const et = endTag || tag;
      return selectedText.startsWith(tag) && selectedText.endsWith(et);
    };
    setActiveStyles({
      bold: check('**'),
      italic: check('*') && !check('**'),
  ...
  const applyStyle = useCallback((type: string) => {
    ...
    if (type === 'table') {
      ...
      const newText = isRemoving 
        ? selectedText.trim().substring(startTag.length, selectedText.trim().length - endTag.length)
        : `${startTag}${selectedText}${endTag}`;
      editorInst.executeEdits("style", [{ range: selection, text: newText, forceMoveMarkers: true }]);
      editorInst.setSelection(new Selection(selection.startLineNumber, selection.startColumn, selection.endLineNumber, selection.endColumn + (newText.length - selectedText.length)));
    }
    editorInst.focus();
